...
export default function FilterBar() {
  ...
  const availableMeters = village ? (metersByVillage[village] || []) : meters;

  return (
    <div className="bg-white rounded-lg shadow">
      <button
        onClick={() => setOpen(!open)}
        className="w-full px-3 py-2.5 flex items-center justify-between text-sm hover:bg-slate-50"
      >
        <div className="flex items-center gap-2">
          ...
          {activeCount > 0 && (
            <span className="bg-brand-100 text-brand-800 px-2 py-0.5 rounded-full text-xs font-semibold">
              {activeCount} active
            </span>
          )}
        </div>
      ...
      {open && (
        <div className="p-3 border-t border-slate-100 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
          ...
          <Field label="From date">
            <input type="date" value={from} onChange={(e) => update('from', e.target.value)} className="filter-input" />
          </Field>
          <Field label="To date">
            <input type="date" value={to} onChange={(e) => update('to', e.target.value)} className="filter-input" />
          </Field>
