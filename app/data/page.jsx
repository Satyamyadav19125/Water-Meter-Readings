...
export default function DataPage() {
  ...
  if (loading) return <div className="h-64 bg-white rounded-xl shadow-sm animate-pulse" />;
  if (error && !stats) return <div className="bg-red-50 border border-red-200 rounded p-3 text-sm text-red-800">{error} — admin only page.</div>;
  ...
  return (
    <div className="space-y-4 max-w-3xl mx-auto">
      <div>
        <h2 className="text-xl font-semibold">🗄️ Data & Storage</h2>
        <p className="text-sm text-slate-500">MongoDB Atlas usage, monthly downloads, and old-data cleanup.</p>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm p-4 sm:p-5">
        <div className="flex items-baseline justify-between mb-1">
          ...
          <div className="text-sm text-slate-600">
            <b>{fmtBytes(used)}</b> of {fmtBytes(limit)} used
          </div>
        </div>
        ...
        <p className="text-[11px] text-slate-400 mt-3">
          Note: your meter <b>readings are stored in KoboToolbox</b>, not here — this database only holds app data (photos, chat, tasks, settings). Deleting things here never deletes any reading.
        </p>
      </div>

      {/* Monthly downloads */}
      <div className="bg-white rounded-2xl shadow-sm p-4 sm:p-5">
        <h3 className="font-semibold mb-1">Monthly downloads</h3>
        <p className="text-xs text-slate-500 mb-3">Archive each month before cleaning up: all readings, flagged-only, and the GPS map file.</p>
        <ul className="divide-y divide-slate-100">
          {months.map((m) => (
            <li key={m.from} className="py-2.5 flex items-center gap-2 flex-wrap">
              <span className="font-medium text-sm flex-1 min-w-[120px]">{m.label}</span>
      ...
      {/* Cleanup */}
      <div className="bg-white rounded-2xl shadow-sm p-4 sm:p-5">
        <h3 className="font-semibold mb-1">🧹 Free up space</h3>
        <p className="text-xs text-slate-500 mb-3">Pick a date, then delete app data OLDER than it. Download your archives first!</p>
        <label className="block text-xs font-medium text-slate-600 mb-1">Delete everything before:</label>
        <input type="date" value={before} onChange={(e) => setBefore(e.target.value)}
          className="px-3 py-2 border border-slate-300 rounded-lg text-sm mb-3" />
        <div className="flex gap-2 flex-wrap">
          <button onClick={() => cleanup('messages', 'chat messages')} disabled={busy}
          ...
          <button onClick={() => cleanup('tasks', 'finished tasks')} disabled={busy}
            className="px-3 py-2 text-sm rounded-lg border border-red-200 text-red-700 hover:bg-red-50 disabled:opacity-50">
            {busy === 'tasks' ? 'Deleting…' : '✅ Finished tasks'}
          </button>
          <button onClick={() => cleanup('verifications', 'old verifications')} disabled={busy}
            className="px-3 py-2 text-sm rounded-lg border border-red-200 text-red-700 hover:bg-red-50 disabled:opacity-50">
      ...
      <style jsx>{`
        :global(.dl-btn) {
          font-size: 0.75rem; padding: 0.375rem 0.625rem; border-radius: 0.5rem;
