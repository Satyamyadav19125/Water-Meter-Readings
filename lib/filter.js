...
/**
 * Return the set of village names this user is allowed to see.
 * Returns null = all (admin or no restriction).
 * Used by the meter-status tracker (village-scoped), NOT by the personal
 * submission views (those are now name-scoped — see filterSubmissionsForUser).
 */
export async function getAllowedVillages() {
  ...
  return new Set(); // user has no assignments -> sees nothing
}

/**
 * Personal submission views (Overview, Submissions, Usage, Map, Analytics).
 * - Admin: sees everything.
 * - Field assistant: sees ONLY their own readings, matched by surveyor name
 *   (case-insensitive + trimmed) against the Kobo "M Name" field.
 *
 * This is what makes Khaushik's dashboard show his own 26 readings instead of
 * all 85 readings taken across his 4 villages by every surveyor.
 */
...
/**
 * URL-based filter pipeline. Applied AFTER user-level filter.
 * Supports query params: village, meter, surveyor, from, to
 */
