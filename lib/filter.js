...
/**
 * VILLAGE-SCOPED view for field assistants.
 * A surveyor sees EVERY submission in their assigned villages — including
 * readings taken by other surveyors in those villages. (Use applyWhoFilter /
 * splitOwn to separate "mine" from "others".)  Admin sees everything.
 */
export async function filterSubmissionsForUser(submissions) {
  ...
  return submissions.filter((s) =>
    allowed.has(String(getField(s, 'village') || '').trim().toLowerCase())
  );
}

/**
 * Split a list into the current user's own readings vs everyone else's,
 * matched by surveyor name (case-insensitive).
 */
export function splitOwn(submissions, userName) {
  const me = String(userName || '').trim().toLowerCase();
  const mine = [];
  const others = [];
  for (const s of submissions) {
    if (String(getField(s, 'surveyor') || '').trim().toLowerCase() === me) mine.push(s);
    else others.push(s);
  }
  return { mine, others };
}
...
/**
 * Apply a who=mine|others|all filter (used by Submissions / Map / Usage).
 */
export function applyWhoFilter(submissions, who, userName) {
  if (!who || who === 'all') return submissions;
  const me = String(userName || '').trim().toLowerCase();
  if (who === 'mine') return submissions.filter((s) => String(getField(s, 'surveyor') || '').trim().toLowerCase() === me);
  if (who === 'others') return submissions.filter((s) => String(getField(s, 'surveyor') || '').trim().toLowerCase() !== me);
  return submissions;
}
