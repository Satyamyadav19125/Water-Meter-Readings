...
// Same-day reads. Groups the SAME meter (+farm) read 2+ times on the SAME field
// DATE — regardless of the reading value — so the admin can find every meter
// submitted twice in a day and decide manually (edit one, or mark the mistaken
// one as dead). This is deliberately BROAD: two reads at different times with
// different values ARE surfaced here for review (they may be legitimate, or one
// may be a mistake — the admin decides). The automatic red flag
// `duplicate_same_day` stays STRICTER (same value only) so it never cries wolf.
// Returns a map id -> [other ids] and a Set of all involved ids. Dead readings
// are INCLUDED so a pair the admin already resolved still shows.
