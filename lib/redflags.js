...
// Same-day duplicates: the SAME meter (on the same farm) read more than once on
...
// whether the duplicate red-flag toggle is on. Dead readings are INCLUDED so a
// pair the admin already resolved (one marked dead) still shows as a duplicate.
  ...
  for (const serial in groups) {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      if (isOptIn('duplicate_same_day')) {
        // Use the FORM date (when the reading was taken), not the upload time.
        // Key on farm+meter+day so two DIFFERENT meters on the same day are never
        // wrongly flagged — only the exact same meter read twice in a day trips.
        const t = readingTime(sub);
        if (!Number.isNaN(t)) {
          const farm = getField(sub, 'farm') || '';
          const dayKey = `${farm}|${serial}|${new Date(t).toISOString().slice(0, 10)}`;
          if (seenDays[dayKey]) {
            push(id, serial, { type: 'duplicate_same_day', severity: 'low', message: `This exact meter (${serial}) was already read on ${new Date(t).toISOString().slice(0, 10)}`, previousSubmissionId: seenDays[dayKey], previousDate: new Date(t).toISOString().slice(0, 10) });
          }
          seenDays[dayKey] = id;
        }
      }
