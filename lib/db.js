...
// Admin can clear a false-positive flag (e.g. two real meters at one spot that
// trip identical_gps). We store the approved submission IDs; the UI then shows
// them green instead of red, and they're excluded from flagged counts.
...
export async function getStorageStats() {
  ...
  markMongoUp();
  return {
    limitBytes: 512 * 1024 * 1024, // MongoDB Atlas free (M0) tier
...
// Deletes OLD app data to free space. Never touches Kobo (readings live in
// Kobo, not here). Supported: chat messages before a date; finished tasks
// done before a date; verifications recorded before a date.
