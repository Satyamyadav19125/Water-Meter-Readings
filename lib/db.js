...
function getClient() {
  ...
  if (!cached) {
    cached = new MongoClient(MONGODB_URI, { serverSelectionTimeoutMS: 5000 });
...
// ---- tiny in-memory cache so settings/assignments aren't re-read on every call ----
...
// ---- Assignments (now CRASH-SAFE: returns [] if DB is unreachable) ----
...
// ---- Media (profile photos stored IN MongoDB, no external service) ----
export async function saveMedia(base64DataUrl) {
  ...
  const data = m[2];
  // ~200KB cap after client-side resize keeps docs small
