// =====================================================================
// FIELD MAP — robust, CASE-INSENSITIVE matching for the WATER-METER form.
//
// Your real Kobo keys (from /debug):
//   group_1/Q2          -> village   (NOTE: capital Q)
//   group_2/meter_id    -> serial
//   group_2/reading     -> reading
//   group_1/m_name      -> surveyor
//   group_2/location    -> GPS
//   group_2/photo_reading -> photo
//   group_1/date        -> date
//
// A `farm` mapping is included too: if your form ever adds a farm question
// the dashboard picks it up automatically (village -> farm -> meter). Until
// then getField(sub,'farm') simply returns null and every farm feature just
// shows nothing — nothing breaks.
//
// getField matches regardless of upper/lowercase AND can match on just the
// last path segment, so small form changes won't break the dashboard.
// "serial" is kept as the internal key for the meter ID so all the shared
// logic (grouping, weekly targets, red flags, map) works unchanged.
// =====================================================================

export const FIELD_MAP = {
  village: ['group_1/Q2', 'group_1/q2', 'group_1/village', 'group_1/village_name', 'Q2', 'q2', 'village', 'village_name'],
  farm:    ['group_2/farm', 'farm', 'group_2/farm_id', 'farm_id', 'group_1/farm'],
  serial:  ['group_2/meter_id', 'meter_id', 'group_2/meter', 'wm_serial', 'serial'],

  // The cumulative meter reading. Both reading and endReading point here
  // because the shared logic reads "endReading" as the current value.
  reading:      ['group_2/reading', 'reading', 'meter_reading'],
  endReading:   ['group_2/reading', 'group_2/end_reading', 'end_reading', 'reading'],
  // Some forms record the previous/start reading in the same visit.
  startReading: ['group_2/start_reading', 'start_reading', 'reading_start'],
  // Water meters have no "outside validation" number (that is a pipe question).
  // Left mapped to nothing that exists so getField() returns null on meter data.
  validation:   ['group_2/validation', 'validation', 'outside_validation'],

  startTime: ['group_1/start_time', 'group_1/Start', 'start_time', 'Start'],
  endTime:   ['group_1/end_time', 'End_time', 'end_time'],
  date:      ['group_1/date', 'group_1/Date', 'date', 'Date', 'today'],

  photo:    ['group_2/photo_reading', 'meter_photo', 'photo'],
  location: ['group_2/location', 'group_2/Location', 'gps', '_geolocation'],

  surveyor: ['group_1/m_name', 'group_1/name', 'm_name', 'name', 'surveyor_name', 'surveyor'],
};

export function getField(submission, key) {
  if (!submission) return null;
  const conf = FIELD_MAP[key];
  if (!conf) return null;
  const candidates = Array.isArray(conf) ? conf : [conf];

  // 1) Exact match
  for (const path of candidates) {
    if (path == null) continue;
    if (path in submission) {
      const v = submission[path];
      if (v !== null && v !== undefined && v !== '') return v;
    }
  }

  // 2) Case-insensitive full-key match
  const wantLower = candidates.map((c) => String(c).toLowerCase());
  for (const k in submission) {
    if (wantLower.includes(k.toLowerCase())) {
      const v = submission[k];
      if (v !== null && v !== undefined && v !== '') return v;
    }
  }

  // 3) Last-path-segment match (e.g. ".../Q2" matches "q2")
  const wantSeg = candidates.map((c) => String(c).split('/').pop().toLowerCase());
  for (const k in submission) {
    const seg = k.split('/').pop().toLowerCase();
    if (wantSeg.includes(seg)) {
      const v = submission[k];
      if (v !== null && v !== undefined && v !== '') return v;
    }
  }

  return null;
}

export function parseReading(value) {
  if (value === null || value === undefined || value === '') return NaN;
  const n = Number(String(value).trim());
  return Number.isFinite(n) ? n : NaN;
}
