...
// Drop the Farm ID column entirely when the form has no farm data, so an empty
// column never shows up in any download.
function hasFarms(submissions) {
  return (submissions || []).some((s) => { const f = getField(s, 'farm'); return f != null && String(f).trim() !== ''; });
}
function activeColumns(submissions) {
  return hasFarms(submissions) ? COLUMNS : COLUMNS.filter((c) => c.key !== 'farm');
}

export function toCsv(submissions) {
  const cols = activeColumns(submissions);
  const header = cols.map((c) => csvEscape(c.label)).join(',');
  const rows = submissions.map((s) => cols.map((c) => csvEscape(c.get(s))).join(','));
...
export function toJson(submissions) {
  ...
  return submissions.map((s) => {
    const row = {};
    for (const c of cols) row[c.key] = c.get(s);
    return row;
  });
}
...
export function toLabeledRows(submissions) {
  ...
  return submissions.map((s) => {
    const row = {};
    for (const c of cols) row[c.label] = c.get(s) ?? '';
    return row;
  });
}
...
export function toCleanLabeledRows(submissions) {
  const withFarm = hasFarms(submissions);
  return submissions.map((s) => {
    const parts = String(getField(s, 'location') || '').trim().split(/\s+/);
    const lat = parts[0] || '';
    const lng = parts[1] || '';
    ...
    if (withFarm) row['Farm ID'] = getField(s, 'farm') ?? '';
    row['Meter ID'] = getField(s, 'serial') ?? '';
    row['Reading'] = getField(s, 'endReading') ?? getField(s, 'reading') ?? '';
    row['Longitude'] = lng;
    row['Latitude'] = lat;
    row['Submission UID'] = s._id;
    return row;
  });
}
