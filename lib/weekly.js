...
// The date a reading is FOR (the form date), falling back to the upload time.
// This is what fixes "late upload of last week's reading counts for this week".
...
// [start, end) for the week BEFORE the given date.
export function lastWeekRange(refDate = new Date()) {
  const thisStart = startOfWeek(refDate);
...
// Core: bucket readings by their READING DATE inside an arbitrary [weekStart, weekEnd).
export function computeStatusForWindow(meters, submissions, weekStart, weekEnd, refDate = new Date()) {
  const buckets = {};
  for (const sub of submissions) {
...
export function computeWeeklyStatus(meters, submissions, refDate = new Date()) {
  return computeStatusForWindow(meters, submissions, startOfWeek(refDate), endOfWeek(refDate), refDate);
}
