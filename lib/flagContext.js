...
import { getSettings, getMeterLocations } from './db.js';
...
export async function buildFlagOptions(settings) {
  ...
  const geofence = s?.meter?.geofence || null;
  let refLocations = null;
  if (geofence?.enabled) {
    try { refLocations = (await getMeterLocations()).locations || {}; } catch { refLocations = {}; }
  }
